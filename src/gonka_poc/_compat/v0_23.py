...
def install_engine_core_poc_methods() -> bool:
    ...
    runs there too (load_general_plugins in vllm/v1/engine/core.py:107-109, v0.23.0), so injecting
    ...
    Upstream symbols pinned here:
        * ``EngineCore`` (vllm/v1/engine/core.py:95) with ``self.scheduler``
          (core.py:149); ``Scheduler.kv_cache_manager``
          (vllm/v1/core/sched/scheduler.py:231)
        * ``KVCacheManager.block_pool`` = the ONE pool shared by every
          kv-cache group (vllm/v1/core/kv_cache_manager.py:156) and
          ``KVCacheManager.kv_cache_config`` (kv_cache_manager.py:157)
        * ``BlockPool.get_new_blocks`` (vllm/v1/core/block_pool.py:333) /
          ``free_blocks`` (block_pool.py:419) / ``get_num_free_blocks``
          (block_pool.py:497); the null block is id 0 and never in the
          free queue (block_pool.py:176-177)
...
async def borrow_poc_blocks(
    ...
    (vllm/v1/engine/core_client.py:1086/:1428, v0.23.0). Raises when that surface is missing or the RPC
